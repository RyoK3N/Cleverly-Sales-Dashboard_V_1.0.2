...
def process_data(dataframes: dict[str, pd.DataFrame], st_date: str,
    ...
    # Apply date filtering to each DataFrame
    op_cancelled_filtered = fdate(op_cancelled, filter_column)
    op_lost_filtered = fdate(op_lost, filter_column)
    op_noshow_filtered = fdate(op_noshow, filter_column)
    op_proposal_filtered = fdate(op_proposal, filter_column)
    op_scheduled_filtered = fdate(op_scheduled, filter_column)
    op_unqualified_filtered = fdate(op_unqualified, filter_column)
    op_won_filtered = fdate(op_won, filter_column)

    # ─────────────────────────────────────────────────────────────────────
    #  RAW COUNTS - Using filtered dataframes
    # ───────────────────────────────────────────────────────────────────────
    all_stages_filtered = pd.concat([
        op_cancelled_filtered, op_lost_filtered, op_noshow_filtered, 
        op_proposal_filtered, op_scheduled_filtered,
        op_unqualified_filtered, op_won_filtered
    ])
    kpi["New Calls Booked"] = (all_stages_filtered.groupby("Owner").size().reindex(kpi.index, fill_value=0))

    sc_taken_df_filtered = pd.concat([op_unqualified_filtered, op_proposal_filtered, op_won_filtered, op_lost_filtered])
    kpi["Sales Call Taken"] = (sc_taken_df_filtered.groupby("Owner").size().reindex(kpi.index, fill_value=0))

    kpi["Unqualified"] = (op_unqualified_filtered.groupby("Owner").size().reindex(kpi.index, fill_value=0))

    kpi["Cancelled Calls"] = (op_cancelled_filtered.groupby("Owner").size().reindex(kpi.index, fill_value=0))

    # Proposals count = Proposal + Won + Lost (using filtered data)
    proposals_df_filtered = pd.concat([op_proposal_filtered, op_won_filtered, op_lost_filtered])
    kpi["Proposals"] = (proposals_df_filtered.groupby("Owner").size().reindex(kpi.index, fill_value=0))

    # ───────────────────────────────────────────────────────────────────────
    #  Origin Counts
    # ───────────────────────────────────────────────────────────────────────
    # Count the number of cold emails in origin column and group by owner
    cold_emails = all_stages_filtered[all_stages_filtered['color__1'] ==
                                      'cold-email']
    ...
    linkedin = all_stages_filtered[all_stages_filtered['color__1'] ==
                                    'LinkedIn']
    ...
    # Close metrics
    closes = op_won_filtered.groupby("Owner").size()
    ...
    # ───────────────────────────────────────────────────────────────────────
    #  REVENUE - Using filtered dataframes
    # ───────────────────────────────────────────────────────────────────────
    def safe_revenue_sum(df: pd.DataFrame) -> pd.Series:
        if df.empty:
            return pd.Series(dtype=float).reindex(kpi.index, fill_value=0.0)
        df_copy = df.copy()
        df_copy["Deal Value"] = pd.to_numeric(df_copy["Deal Value"], errors="coerce").fillna(0)
        return (df_copy.groupby("Owner")["Deal Value"].sum().reindex(kpi.index, fill_value=0.0))

    kpi["Closed Revenue $"] = safe_revenue_sum(op_won_filtered)

    # Pipeline Revenue = Proposal stage revenue (filtered)
    kpi["Pipeline Revenue $"] = safe_revenue_sum(op_proposal_filtered)
